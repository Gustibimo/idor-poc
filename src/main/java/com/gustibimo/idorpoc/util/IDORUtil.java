package com.gustibimo.idorpoc.util;
...
public class IDORUtil {
     ...
     *
     * @param realItemBackendId Identifier of the item on the backend storage (real identifier)
    ...
    public static String computeFrontEndIdentifier(String realItemBackendId) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        String frontEndId = null;
        if (realItemBackendId != null && !realItemBackendId.trim().isEmpty()) {
            //Prefix the value with the SALT
            String tmp = SALT + realItemBackendId;
